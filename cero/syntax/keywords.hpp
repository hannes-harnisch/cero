...
consteval auto get_keyword_tails() {
	static constexpr size_t count = (size_t) std::ranges::count_if(keywords,
	    [](Keyword keyword) { return keyword.lexeme.starts_with(first); });

	std::array<KeywordTail, count> tails;
	size_t i = 0;
	for (const Keyword& keyword : keywords) {
		if (keyword.lexeme.starts_with(first)) {
			KeywordTail& tail = tails[i++];
			tail.tail = encode_keyword_tail(keyword.lexeme);
			tail.kind = keyword.kind;
		}
	}
	return tails;
}
