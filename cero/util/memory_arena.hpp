...
class ArenaArray {
public:
	void add(MemoryArena& arena, const T& value) {
		const uint32_t count = alloc_->count;
		const uint32_t capacity = alloc_->capacity;

		if (count == capacity) {
			uint32_t new_cap = std::max(capacity + capacity / 2, initial_capacity);
			void* storage = arena.allocate(sizeof(Allocation) + new_cap, alignof(Allocation));

			Allocation* new_alloc = new (storage) Allocation;
			new_alloc->count = count;
			new_alloc->capacity = new_cap;
