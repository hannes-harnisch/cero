...
#include <cero/util/push_state.hpp>
...
void AstCompare::visit_function_definition(const cero::AstFunctionDefinition& fn_def) {
	...
	CHECK_EQ(str_.get(), get_name(fn_def.name));
}
...
void AstCompare::visit_function_parameter(const cero::AstFunctionParameter& param) {
	...
	CHECK_EQ(str_.get(), get_name(param.name));
}
...
void AstCompare::visit_function_output(const cero::AstFunctionOutput& output) {
	...
	CHECK_EQ(str_.get(), get_name(output.name));
}

void AstCompare::id_expr(std::string_view id) {
	record(cero::AstNodeKind::id_expr);

	str_.set(id);
	visit_node();
}

void AstCompare::visit_id_expr(const cero::AstIdExpr& id_expr) {
	expect(id_expr, {});

	std::string_view ident = source_.substr(id_expr.offset, id_expr.length);
...
void AstCompare::visit_literal_expr(const cero::AstLiteralExpr& literal_expr) {
	...
	std::string_view literal = source_.substr(literal_expr.offset, literal_expr.length);
...
void AstCompare::visit_node(ChildScope child_scope) {
	...
	cero::PushState rcc(recorded_child_count_, 0);
	cero::PushState nc(next_children_, std::move(expected_children));
...
std::string_view AstCompare::get_name(cero::AstName name) {
	return source_.substr(name.offset, name.length);
}
