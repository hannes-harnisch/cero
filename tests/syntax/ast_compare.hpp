...
class AstCompare : public cero::AstVisitor<AstCompare> {
public:
...
private:
	...
	void visit_function_definition(const cero::AstFunctionDefinition& fn_def);
	...
	void visit_binary_expr(const cero::AstBinaryExpr& binary_expr);
	void visit_call_expr(const cero::AstCallExpr& call_expr);
	...
	std::string_view get_name(cero::AstName name);

	friend class cero::AstVisitor<AstCompare>;
};
