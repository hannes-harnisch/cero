...
class Parser {
public:
	Parser(const TokenStream& token_stream, const SourceGuard& source, Reporter& reporter, uint8_t tab_size) :
		ast_(token_stream),
...
private:
	...
	void on_trailing_name(SourceOffset offset, Ast::NodeIndex prev_expr, StringId name, SourceOffset name_offset) {
		...
		if (!std::ranges::contains(type_expr_kinds, kind)) {
			auto location = source_.locate(name_offset, tab_size_);
			report(Message::NameCannotAppearHere, location, MessageArgs());
			throw ParseError();
		}
	...
	Ast::NodeIndex on_if_expr() {
		...
		if (auto colon = cursor_.match_token(TokenKind::Colon)) {
			if (cursor_.peek_kind() == TokenKind::LBrace) {
				report(Message::UnnecessaryColonBeforeBlock, colon->locate_in(source_, tab_size_), MessageArgs());
			}
		} else {
			if (cursor_.peek_kind() != TokenKind::LBrace) {
	...
	bool lookahead_whether_is_generic() {
		...
		if (cursor_.match(TokenKind::RAngle)) {
			...
			return (is_binding_allowed_ || next != TokenKind::Name)
				   ...
				   && !std::ranges::contains(ForbiddenAfterGeneric, next)
	...
	void check_binary_operator_ambiguity(BinaryOperator left, BinaryOperator right, Token operator_token) {
		if (operators_are_ambiguous(left, right)) {
			auto location = operator_token.locate_in(source_, tab_size_);
	...
	static bool operators_are_ambiguous(BinaryOperator left, BinaryOperator right) {
		...
		case Div:
		case Rem:
		case Pow:	   return std::ranges::contains(bitwise_operators, right);
		case BitAnd:
		case BitOr:
		case Xor:
		case Shl:
		case Shr:	   return std::ranges::contains(arithmetic_operators, right);
		...
		case LessEq:
		case GreaterEq:
			return std::ranges::contains(comparison_operators, right)
				   && !std::ranges::contains(transitive_comparisons, OperatorPair {left, right});
	...
	void parse_function_type_parameter() {
		...
		if (auto equal = cursor_.match_token(TokenKind::Eq)) {
			auto location = equal->locate_in(source_, tab_size_);
			report(Message::FuncTypeDefaultArgument, location, MessageArgs());
			throw ParseError();
		}
	}
	...
	void report_expectation(Message message) {
		auto token = cursor_.peek();
		auto location = token.locate_in(source_, tab_size_);
...
Ast run_parser(const TokenStream& token_stream, const SourceGuard& source, Reporter& reporter, uint8_t tab_size) {
	return Parser(token_stream, source, reporter, tab_size).parse();
}

Ast run_parser_on_source(const SourceGuard& source, Reporter& reporter, uint8_t tab_size) {
	auto token_stream = run_lexer(source, reporter, false, tab_size);
	return run_parser(token_stream, source, reporter, tab_size);
}
