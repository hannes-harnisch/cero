...
class Lexer {
public:
	Lexer(const SourceGuard& source, Reporter& reporter, bool emit_comments, uint8_t tab_size) :
		stream_(source),
		cursor_(source),
		source_(source),
		reporter_(reporter),
	...
	TokenStream lex() && {
		if (source_.get_length() > MaxSourceLength) {
			const CodeLocation blank {source_.get_name()};
			reporter_.report(blank, Message::SourceFileTooLarge, MessageArgs(MaxSourceLength));
		} else {
			lex_source();
		}
...
private:
	...
	void eat_block_comment(SourceOffset offset) {
		...
		report(Message::UnterminatedBlockComment, offset, MessageArgs());
	}
	...
	void lex_quoted_sequence(TokenKind kind, SourceOffset offset, char quote) {
		...
		while (auto next = cursor_.peek()) {
			const char c = *next;
			if (c == '\n') {
				report(Message::MissingClosingQuote, cursor_.offset(), MessageArgs());
				break;
			}
	...
	void report(Message message, SourceOffset offset, MessageArgs args) {
		auto location = source_.locate(offset, tab_size_);
...
TokenStream run_lexer(const SourceGuard& source, Reporter& reporter, bool emit_comments, uint8_t tab_size) {
	return Lexer(source, reporter, emit_comments, tab_size).lex();
}
